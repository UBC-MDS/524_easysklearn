...
def baseline_fun(X_train, y_train, type = 'regression', metrics_1 = 'accuracy', metrics_2 = 'r2'):
    ...
    if type not in ['regression', 'classification']:
        raise TypeError('Please check what kind of supervised machine learning to use, regression or classification')

    if metrics_1 not in sklearn.metrics.SCORERS.keys():
        raise KeyError('Please check sklearn.metrics.SCORERS.keys() to get valid options')
    ...
    score = {}
    if type == 'regression':
            dr_score = cross_validate(DummyRegressor(), X_train, y_train, return_train_score=True, scoring=metrics_2)
            lr_score = cross_validate(LinearRegression(), X_train, y_train, return_train_score=True, scoring=metrics_2)
            score["DummyRegressor"] = pd.DataFrame(dr_score).mean()
            score["LinearRegression"] = pd.DataFrame(lr_score).mean()
    if type == 'classification':
            dc_score = cross_validate(DummyClassifier(), X_train, y_train, return_train_score=True, scoring=metrics_1)
            lr_score = cross_validate(LogisticRegression(), X_train, y_train, return_train_score=True, scoring=metrics_1)
