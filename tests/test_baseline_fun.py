...
def test_input_exception():
    ...
    assert (str(exc.value) == "'Please check sklearn.metrics.SCORERS.keys() to get valid options'")
...
def test_output():
    ...
    assert (score_r.index == ['fit_time', 'score_time', 'test_score', 'train_score']).all()
    assert (score_c.index == ['fit_time', 'score_time', 'test_score', 'train_score']).all()
